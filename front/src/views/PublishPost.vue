...
<template>
  <section class="section">
    ...
    <form class="card list-editor" @submit.prevent="submit">
      <label class="field"><span>标题</span><input v-model.trim="form.title" maxlength="80" required /></label>
      <label class="field"><span>内容</span><textarea v-model.trim="form.content" rows="7" maxlength="800" required></textarea></label>
      ...
      <button class="btn" type="submit" :disabled="loading">{{ loading ? "发布中..." : "发布动态" }}</button>
    </form>
  </section>
</template>
