...
<template>
  <section class="section checkout-page">
    <div class="section-head">
      <div>
        <h2>购物车</h2>
        <p class="lead">支持数量加减、删除、全选/单选、小计、优惠和应付金额实时更新。</p>
      </div>
      <RouterLink class="btn ghost" to="/shop">继续选购</RouterLink>
    </div>

    <div v-if="cartStore.items.length" class="cart-workbench">
      <div class="card cart-panel premium-cart">
        ...
        <div v-for="item in cartStore.items" :key="item.productId" class="cart-row cart-item-row">
          ...
          <div class="cart-quantity">
            ...
            <input :value="item.quantity" type="number" min="1" :max="item.stock || 1" step="1" @change="changeQuantity(item, $event.target.value, $event.target)" />
      ...
      <aside class="card checkout-summary-card">
        ...
        <button class="btn checkout-main-btn" type="button" :disabled="!cartStore.selectedItems.length" @click="checkout">去结算</button>
      </aside>
    </div>

    <div v-else class="card empty">
      <p class="muted">购物车还是空的，先去文创商城挑选商品吧。</p>
